...
- **Step 2 is TWO parted commands: `mklabel gpt` AND `mkpart`.** If you run only `mklabel` (or run it twice) and skip `mkpart`, no partition is created → `lsblk /dev/sdb` shows no `sdb1` → `mkfs.ext4 /dev/sdb1` fails with `The file /dev/sdb1 does not exist`. `partprobe` cannot create a partition that was never made — re-run `sudo parted -s /dev/sdb mkpart primary ext4 0% 100%`, then `partprobe` + `mkfs`.
- **It's fine if the node you stop turns out to be the Leader.** `systemctl stop patroni` on the leader does a graceful, zero-data-loss failover (a replica is promoted, timeline `TL` increments); the stopped node later rejoins as a Replica. Node order is flexible — the only hard rules are **one node at a time** and **keep a Leader + at least one healthy standby up**. If you accidentally stop the leader, just finish that node's migration; it comes back as a replica.
